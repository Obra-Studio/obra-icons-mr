...
figma.ui.onmessage = async (msg) => {
  ...
  if (msg.type === 'auto-colour-named-layers') {
    ...
    if (msg.type === 'apply-simplify') {
        ...
        figma.notify(`Simplified ${vectors.length} vector(s) with tolerance ${tolerance}`);
    }
...
function reportOnGroups(selection = figma.currentPage.selection) {
    ...
    if (groupsFound > 0) {
        ...
        figma.notify(`Found ${groupsFound} group(s) in ${layersWithGroups.length} layer(s). Check console for details.`);
    } else {
...
function renderBezierRecursive(
    ...
    points, distanceTolerance, level
) {
    ...
    renderBezierRecursive(
        ...
        points, distanceTolerance, level + 1
    );
    renderBezierRecursive(
