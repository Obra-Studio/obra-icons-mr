<!-- ! This file is automatically generated, please don't edit it directly. -->

<svelte:options namespace="svg" />

<script lang="ts">
  export let size = 24
  export let color = 'currentColor'
</script>

<svg
	width={size}
	height={size}
	viewBox="0 0 24 24"
	fill="none"
	xmlns="http://www.w3.org/2000/svg"
	class="obra-icon"
>
	<g class="oi-notification-fill">
		<path
			class="oi-fill"
			fill-rule="evenodd"
			clip-rule="evenodd"
			d="M4.99997 10C4.99997 6.13401 8.13398 3 12 3C15.866 3 19 6.13401 19 10V14.6972L20.832 17.4453C21.0366 17.7522 21.0557 18.1467 20.8816 18.4719C20.7076 18.797 20.3688 19 20 19H3.99997C3.63118 19 3.29232 18.797 3.1183 18.4719C2.94428 18.1467 2.96335 17.7522 3.16792 17.4453L4.99997 14.6972V10Z"
			fill={color}
		/>
		<path
			class="oi-fill"
			fill-rule="evenodd"
			clip-rule="evenodd"
			d="M9.49995 17.5C10.0522 17.5 10.5 17.9477 10.5 18.5C10.5 19.3284 11.1715 20 12 20C12.8284 20 13.5 19.3284 13.5 18.5C13.5 17.9477 13.9477 17.5 14.5 17.5C15.0522 17.5 15.5 17.9477 15.5 18.5C15.5 20.433 13.9329 22 12 22C10.067 22 8.49995 20.433 8.49995 18.5C8.49995 17.9477 8.94767 17.5 9.49995 17.5Z"
			fill={color}
		/>
		<path
			class="oi-fill"
			fill-rule="evenodd"
			clip-rule="evenodd"
			d="M9.99995 4C9.99995 2.89543 10.8954 2 12 2C13.1045 2 14 2.89543 14 4C14 4.55228 13.5522 5 13 5C12.4477 5 12 4.55228 12 4C12 4.55228 11.5522 5 11 5C10.4477 5 9.99995 4.55228 9.99995 4Z"
			fill={color}
		/>
	</g>
</svg>
