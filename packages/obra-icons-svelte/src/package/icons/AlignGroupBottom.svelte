<svelte:options namespace="svg" />

<script>
  export let size = 24
  export let color = 'currentColor'
  // svelte-ignore unused-export-let
  export let strokeWidth = 2
</script>

<svg
	width={size}
	height={size}
	viewBox="0 0 24 24"
	fill="none"
	xmlns="http://www.w3.org/2000/svg"
>
	<g class="oi-align-group-bottom">
		<path
			class="oi-line"
			d="M2.5 19.5L20.5 19.5"
			stroke={color}
			stroke-width={strokeWidth}
			stroke-linecap="round"
			stroke-linejoin="round"
		/>
		<path
			class="oi-box"
			d="M17.5 15.5L17.5 9.5C17.5 8.94772 17.0523 8.5 16.5 8.5L14.5 8.5C13.9477 8.5 13.5 8.94772 13.5 9.5L13.5 15.5C13.5 16.0523 13.9477 16.5 14.5 16.5H16.5C17.0523 16.5 17.5 16.0523 17.5 15.5Z"
			stroke={color}
			stroke-width={strokeWidth}
			stroke-miterlimit="10"
			stroke-linecap="round"
			stroke-linejoin="round"
		/>
		<path
			class="oi-box"
			d="M9.5 15.5L9.5 5.5C9.5 4.94772 9.05228 4.5 8.5 4.5L6.5 4.5C5.94772 4.5 5.5 4.94771 5.5 5.5L5.5 15.5C5.5 16.0523 5.94771 16.5 6.5 16.5H8.5C9.05228 16.5 9.5 16.0523 9.5 15.5Z"
			stroke={color}
			stroke-width={strokeWidth}
			stroke-miterlimit="10"
			stroke-linecap="round"
			stroke-linejoin="round"
		/>
	</g>
</svg>
