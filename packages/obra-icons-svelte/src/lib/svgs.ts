...
function prepareSvg(svg: string, strokeWeight: number, color: string, size: number) {
    ...
    Object.entries(classStrokeWidths).forEach(([className, initialWidth]) => {
        ...
        const regex = new RegExp(`class="${className}"([^>]*stroke-width=")([^"]*)`, 'g');
        svg = svg.replace(regex, `class="${className}"$1${adjustedWidth.toFixed(2)}`);
    });
...
export function getSvg(name: string, strokeWeight: number, color: string, size: number) {
	return prepareSvg(svgs[`./svgs/${name}.svg`] || '', strokeWeight, color, size);
}